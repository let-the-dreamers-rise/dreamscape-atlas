...
const NeuralSovereignty = () => {
  ...
  const toggleConsent = async (key: keyof typeof consent) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const { data: dreamSymbols } = await supabase.from("dream_symbols").select("*");
      ...
      const exportData = {
        ...
        metadata: { totalDreams: dreams?.length || 0, totalSymbols: symbols?.length || 0 },
      };
      ...
      await supabase.from("data_consent_log").insert({
        user_id: user.id, action: "DATA_EXPORT", scope: "full_export", details: { format: "json" },
      });
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      await (supabase.from("memory_clusters" as any).delete() as any).eq("user_id", user.id);
      await supabase.from("data_consent_log").insert({
        user_id: user.id, action: "DATA_DELETION", scope: "all_neural_data",
  ...
  const consentItems = [
    { key: "aiAnalysis" as const, icon: Brain, title: "AI Dream Analysis", description: "Allow AI to analyze dream descriptions for symbols, emotions, and themes. When revoked, dreams are saved without AI processing.", color: "hsl(var(--primary))", enforcement: "Blocks analyze-dream edge function" },
    { key: "imageGeneration" as const, icon: Eye, title: "Image Generation", description: "Allow AI to generate visual reconstructions from dream descriptions. When revoked, no images are generated.", color: "hsl(var(--dream-accent-violet))", enforcement: "Skips image generation pipeline" },
    { key: "patternDetection" as const, icon: FileText, title: "Pattern Detection", description: "Allow the system to detect recurring patterns across your dreams. When revoked, pattern insights are hidden.", color: "hsl(var(--dream-accent-cyan))", enforcement: "Hides pattern analysis engine" },
    { key: "clusterFormation" as const, icon: Brain, title: "Memory Consolidation", description: "Allow hippocampal clustering engine to form memory clusters. When revoked, consolidation engine is disabled.", color: "hsl(var(--dream-accent-amber))", enforcement: "Disables consolidation algorithm" },
  ];
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}>
          ...
          <div className="mb-12">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "linear-gradient(135deg, hsl(var(--dream-accent-rose) / 0.25), hsl(var(--primary) / 0.15))", border: "1px solid hsl(var(--dream-accent-rose) / 0.4)", boxShadow: "0 0 20px hsl(var(--dream-accent-rose) / 0.15)" }}>
                <Shield className="w-3.5 h-3.5" style={{ color: "hsl(var(--dream-accent-rose))" }} />
              </div>
              <span className="text-[11px] font-semibold tracking-[0.2em] uppercase" style={{ color: "hsl(var(--dream-accent-rose) / 0.8)" }}>Cognitive Liberty</span>
            </div>
            <h1 className="text-4xl sm:text-5xl font-display font-bold tracking-[-0.02em] text-foreground mb-4 leading-[0.95]">
              Neural Data<br /><span className="dream-text-gradient">Sovereignty</span>
            </h1>
            <p className="text-sm text-muted-foreground max-w-lg leading-relaxed">
              ...
              <strong className="text-foreground"> Every toggle below enforces real permissions.</strong>
            </p>
          </div>
          ...
          <div className="mb-10">
            ...
            <div className="space-y-3">
              {consentItems.map((item) => (
                <motion.div key={item.key} className="p-5 rounded-xl dream-glass-strong relative overflow-hidden" whileHover={{ scale: 1.005 }}>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: `${item.color}15`, border: `1px solid ${item.color}25` }}>
                        <item.icon className="w-4 h-4" style={{ color: item.color }} />
                      </div>
                      <div>
                        <h3 className="font-display font-bold text-sm text-foreground">{item.title}</h3>
                        <p className="text-[11px] text-muted-foreground">{item.description}</p>
                        <p className="text-[9px] font-mono mt-1" style={{ color: consent[item.key] ? "hsl(var(--dream-accent-cyan))" : "hsl(var(--destructive))" }}>
                          {consent[item.key] ? "✓ Active" : "✗ Blocked"} — {item.enforcement}
                        </p>
                      </div>
                    </div>
                    <button
                      ...
                      style={{ background: consent[item.key] ? item.color : "hsl(var(--muted))", boxShadow: consent[item.key] ? `0 0 15px ${item.color}40` : "none" }}
                    >
                      <div className="absolute top-1 w-5 h-5 rounded-full bg-white transition-all duration-300" style={{ left: consent[item.key] ? "calc(100% - 24px)" : "4px" }} />
                    </button>
                  </div>
                </motion.div>
              ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
            <button onClick={handleExport} disabled={exporting} className="p-5 rounded-xl dream-glass-strong text-left relative overflow-hidden group transition-all duration-300 hover:scale-[1.01]">
              ...
              <p className="text-[11px] text-muted-foreground">Download complete neural data as JSON.</p>
            </button>
            <button onClick={() => setDeleteConfirm(true)} className="p-5 rounded-xl dream-glass-strong text-left relative overflow-hidden group transition-all duration-300 hover:scale-[1.01]">
          ...
          {/* Delete Confirmation */}
          {deleteConfirm && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-10 p-6 rounded-2xl relative overflow-hidden" style={{ background: "hsl(var(--destructive) / 0.05)", border: "1px solid hsl(var(--destructive) / 0.3)" }}>
              <div className="flex items-center gap-2 mb-3">
                <AlertTriangle className="w-4 h-4 text-destructive" />
                <span className="font-display font-bold text-sm text-foreground">Confirm Permanent Deletion</span>
              </div>
              <p className="text-xs text-muted-foreground mb-4">This will permanently delete all your dreams, symbols, memory clusters, and associated neural data. We recommend exporting your data first.</p>
              <div className="flex gap-3">
                <button onClick={handleDeleteAll} disabled={deleting} className="px-4 py-2 rounded-lg text-xs font-bold text-destructive-foreground transition-all" style={{ background: "hsl(var(--destructive))" }}>
                  {deleting ? "Deleting..." : "Yes, Delete Everything"}
                </button>
                <button onClick={() => setDeleteConfirm(false)} className="px-4 py-2 rounded-lg text-xs font-bold text-foreground dream-glass">Cancel</button>
              </div>
            </motion.div>
          )}
